...
public class Student {
    ...
    private void setStudentEditProfileClick(Event event){
        setAllEnable();
        String sqlQuery = "select * from  student where dbStudentID = "+this.ID+" ;";
        try {
            connection = database.getConnection();
            statement = connection.createStatement();
    ...
    private void setStudentRegistrationClick(Event event)throws IOException{
        ...
        re.setID(ID);
        FXMLLoader loader = new FXMLLoader();
        loader.setLocation(this.getClass().getResource("/student/Registration.fxml"));
        loader.load();
        ...
        registrationController.setTanother(this.studentRunningCourseTableView);
        stage.show();
